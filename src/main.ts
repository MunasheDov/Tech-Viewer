...
let unitSprites: HTMLImageElement[] = [];
let structureSprites: Map<string, HTMLImageElement> = new Map<string, HTMLImageElement>();
...
interface Tech extends TechRaw {
    id: number;
    cost: number;
    spriteId?: number;
    category?: string;
...
class Cell {
    constructor(public tree: Array<Array<Cell>>, public tech: Tech, public indices: Vec, public targetIndices: Vec, public isHidden: boolean) {}
...
interface Structure { id: number, name: string, techId: number, extra: string, filename?: string };

let dataStructuresList: Structure[] = [
...
let dataStructures: Structure[] = new Array(200);
...
function initTechnology(data: TechRaw[]) {
    ...
    for (let i = 0; i < 100; ++i) Categories.push("");
    ...
    technology.forEach((tech, index) => {
        for (let i = 0; i < Categories.length; ++i) {
            ...
            if (index >= technology.indexOf(techDict.get(Categories[i]))) tech.category = Categories[i];
        }
    });
    ...
    for (let i = colorsDefault.length; i < CategoriesUsed; ++i) {
        let key = i.toString();
        if (i < CategoriesUsed) key = Categories[i];
        console.log(key);
...
let units: UnitRaw[] = [];
let unitDict: Map<String, UnitRaw> = new  Map<String, UnitRaw>();

function initUnits(data: UnitRaw[]) {
    if (data.length === 0) throw new Error("data length is 0");
    units = [];
    data.forEach((r, i) => {
...
function cost(tech: Tech | TechRaw) { return tech.stats[3]; };
...
function getDependencies(tech: Tech | TechRaw) {
    let deps: Tech[] = [];
    for (let i = 0; i < 3; ++i) {
...
function generateTreeRequirements(root: Tech) {
    ...
    let arr: Tech[] = []
    if (root.requires) arr = arr.concat(root.requires);
    else {
        let tree: Cell[][] = [[]];
        tree[0].push(new Cell(tree, root, {x: 0, y: 0}, {x: 0, y: 0}, false));
        return tree;
    ...
    for (let i = arr.length-1; i >= 0; --i) {
        ...
        let insert = true;
        pathList.forEach(e => { if (arrayEquals(e, path)) insert = false });
        if (insert)
            pathList.push(path);
    }
    ...
    if (uniqueOnly) {
        for (let j = 0; j < pathList.length; ++j) {
            ...
            for (let i = pathList.length-1; i > j; --i) {
                let current: Array<Tech | null> = pathList[i];
                let remove = false;
                for (let n = 0; n < current.length; ++n) {
                    // flag the curerrent path for removal
                    if (longest.includes(current[n])) remove = true;
                    break;
                }
                if (remove) removeList.push(i);
            }
    ...
    if (tree.length === 0 || tree[0].length === 0) debugger;
    return tree;
}



function pathString(path: Tech[]) {
    let str = "";
    let sep = " <- ";
    path.reverse();
...
function getPathCost(path: Array<Cell>) {
    return path.map(t => !t.isHidden ? t.tech?.cost : 0 ).reduce((a, b) =>  addNumberOrUndefined(a, b));
}

function getTreeCost(tree: Array<Array<Cell>>) {
    return tree.map(t => getPathCost(t)).reduce((a, b) => addNumberOrUndefined(a, b));
}

function addNumberOrUndefined(a: number | undefined, b: number | undefined): number {
    return (a ?? 0) + (b ?? 0);
}


function renderTreeRequirements(x: number, y: number, tree: Array<Array<Cell>>) {
    let root = tree[0][0];
...
function generateStyle(tech: Tech) {
    let style = "";
...
function draw() {
    ...
    if (hoveredTech) drawTooltip(hoveredTech);
    
}


function drawTooltip(tech: Tech) {
    ctx.save();
    let x = mouseX + 20;
    let y = mouseY + 12;
    ...
    if (tech && tech.extra) tooltip += tech.extra;
    if (structure && structure.extra) tooltip += ("\n" + structure.extra);
...
function stringToWrapped(text: string, maxWidth: number) {
    let initalWidth = ctx.measureText(text);
    
    if (initalWidth.width < maxWidth) return text;
...
function generateTreeLeadsTo(rootTech: Tech): Cell[][] {
    let tree: Cell[][] = [[]];
    ...
    for (let i = 0; i < cols; ++i) tree.push([]);
...
function expandCell(c: Cell): Cell[] {
    return c.tech.leadsTo?.map(t => new Cell(c.tree, t, {x: c.indices.x + 1, y: 0}, {x: c.indices.x, y: 0}, false)) ?? [];
}



function renderTreeLeadsTo(originX: number, originY: number, tree: Cell[][]) {
    let biggestY = 0;
...
function drawTechBox(x: number, y: number, w: number, h: number, tech: Tech) {
...
function drawTechLabel(x: number, y: number, tech: Tech) {
    ctx.font = fontString;
    ...
    if (showChurchLike) {
        let cl = tech.stats[4];
        if (cl < 10) ctx.fillStyle = churchDisdainColor;
        if (cl < 5)  ctx.fillStyle = churchHateColor;
        if (cl < 3)  ctx.fillStyle = churchRepugnantColor;
    }
...
function nullVec(v: Vec) {
    return v.x === 0 && v.y === 0;
}

function equalVec(v1: Vec, v2: Vec) {
...
function controlText(control: string, text: string, x: number, y: number) {
...
function textShadowed(text: string, primary: string, secondary: string, x: number, y: number) {
...
let hoveredTech: Tech | null = null;
...
document.addEventListener("keyup", e => {
    if (e.key == "t") showAllTotalCostsInGridMode = !showAllTotalCostsInGridMode;
    if (e.key == "h") showChurchLike = !showChurchLike;
    if (e.key == "Home") {
...
function parseDatTech(dat: String) {
    let dataLoaded: TechRaw[] = [];
    let lines = dat.split('\n');
    lines.forEach(l => {
...
function parseDatUnit(dat: String) {
    let dataLoaded: UnitRaw[] = [];
    let lines = dat.split('\n');
    let slot = 0;
    lines.forEach(l => {
        if (l.charAt(0) === '{') {
...
function parseUnitRow(row: String, slot: number) {
    let tok = row.split("\"");
    tok = tok.filter(t => t.length !== 0 && t.trim().length !== 0);

    let stats = tok[6].split(" ")?.filter(n => n.length !== 0 && !isAlpha(n)).map(Number);

    let unit: UnitRaw = {slot: slot, name: tok[1], suffix: parseInt(tok[2]), abbrev: tok[4], stats: stats};
